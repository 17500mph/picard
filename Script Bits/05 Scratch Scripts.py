...
%artist%/%album%/D%discnumber%/%tracknumber% - %title%

%artist%/%album%/$pad(D%discnumber%,2,0)/$pad(%tracknumber%,2,0) - %title%
...
$if($eq(%_devMode%,1)$set(_bitsPerSample,[%_bits_per_sample%]bit),$set(_bitsPerSample,[%_biits_per_sample%]bit))
$if($eq(%_devMode%,1)$set(_audioChannels,%_channels%ch]),$set(_audioChannels,%_chaannels%ch]))
$if($eq(%_devMode%,1)$set(_titleForFilename,%_title%),$set(_titleForFilename,%_tiitle%))
...
{%_bitrate% - %_sample_rate% - %_bits_per_sample% - %_channels%}

$if($eq(%_devMode%,1)$set(_bitRateSpeed,%_sample_rate%KHz ),$set(_bitRateSpeed,%_saample_rate%KHz ))
$if($eq(%_devMode%,1)$set(_bitRateValue,$left(,3)%_bitrate%),$set(_bitRateValue,$left(,3)%_biitrate%))
$if($eq(%_devMode%,1)$set(_bitsPerSample,[%_bits_per_sample%]bit),$set(_bitsPerSample,[%_biits_per_sample%]bit))
$if($eq(%_devMode%,1)$set(_audioChannels,%_channels%ch]),$set(_audioChannels,%_chaannels%ch]))
$if($eq(%_devMode%,1)$set(_titleForFilename,%_title%),$set(_titleForFilename,%_tiitle%))

{%_bitRateValue% - %_bitRateSpeed% - %_bitsPerSample% - %_audioChannels%}
...
$noop(★ 00 CREDITS ★)
    ...
    3rd Works Properly - With Status Variable Snippets
...
{%title% - %_bitrate% - %_sample_rate% - %_bits_per_sample% - %_channels%}

$if($eq(%_devMode%,1)$set(_bitRateSpeed,%_sample_rate%KHz ),$set(_bitRateSpeed,%_saample_rate%KHz ))
$if($eq(%_devMode%,1)$set(_bitRateValue,$left(,3)%_bitrate%),$set(_bitRateValue,$left(,3)%_biitrate%))
$if($eq(%_devMode%,1)$set(_bitsPerSample,[%_bits_per_sample%]bit),$set(_bitsPerSample,[%_biits_per_sample%]bit))
$if($eq(%_devMode%,1)$set(_audioChannels,%_channels%ch]),$set(_audioChannels,%_chaannels%ch]))
$if($eq(%_devMode%,1)$set(_titleForFilename,%title%),$set(_titleForFilename,%_tiitle%))

{%title% - %_bitRateValue% - %_bitRateSpeed% - %_bitsPerSample% - %_audioChannels% - %_titleForFilename%}

$noop(Bitrate factors of 8.0 are CBR, Anything else is VBR)

$set(_bitRateType,$if($eq_any(%_bitRateValue%,320,256,224,192,160,128,112,96,80,64,56,48,40,32,24,16,8),CBR$set(_cbrRateValue,%_bitRateValue%),VBR$set(_vbrRateValue,%_bitRateValue%)))

{%_fileCBRRate%-%_fileVBRRate%}
